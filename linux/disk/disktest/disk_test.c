...
#define VERSION_STRING	"2015.09.17 v1.2"
...
void print_usage(void)
{
    ...
    printf("-i start count   , default %d                                   \n", START_COUNT);
    printf("-f loop          ,                                              \n");
...
int main(int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, "hrwp:b:d:l:c:i:sftno:"))) {
		switch(opt) {
		...
		case 'i':   start_count = atoi(optarg);		break;
	...
	if (buf_ran) {
		...
		if (buf_len > B_MAX) {
			printf("Fail, Invalid buffer len %d, max %d byte\n", buf_len, B_MAX);
			print_usage();
			perror("error:");
			exit(EXIT_FAILURE);
		}
	...
	/* current disk avail */
	disk_free = get_disk_avail(path, NULL, 1);
	...
	do {
		for (index = 0; test_count > index; index++) {
			...
			sprintf(file, "%s/%s.%d.txt", path, FILE_PREFIX, index+start_count);
			...
			if (opt_rd) {
				...
				if (0 > readl) {
					fprintf(stderr, "read test failed (%lld)\n", readl);
					exit(EXIT_FAILURE);
				}
								...
								long long *plength, int *pbuflen, long *time)
{
	...
	while (count > 0) {
		if (0 > (ret = write(fd, pbuf, count))) {
			...
			if (e == ENOSPC) {
				printf ("Not enough space. retry ...\n");
				sleep (5);
				r = obtain_test_free (path, test_count, length);
				if (0 > r) {
					DBGOUT("No space left on device, free %lld, req %lld\n",
							get_disk_avail(path, NULL, 0), length);
					perror("error:");
					exit(EXIT_FAILURE);
				}
				continue;
