...
/**/    int phase_mask;
/**/    int linear;
		FELEM   *Filter55, *Filter56, *Filter60, *Filter61, *Filter120, *Filter121;
/**/}AVResampleContext;
/**/
...
/**/	#else
/**/	double x, y, w;
/**/	double tab[128];	/* prevant 'warning: the frame size of 1048 bytes is larger than 1024 bytes' */
/**/	#endif
/**/    const int center= (tap_count-1)/2;
/**/
/**/	//tab = MES_MALLOC( tap_count * sizeof(double) );
...
{
	if (c->Filter55)	av_free((void*)c->Filter55);
	if (c->Filter56)	av_free((void*)c->Filter56);
   	if (c->Filter60)	av_free((void*)c->Filter60);
	if (c->Filter61)	av_free((void*)c->Filter61);
   	if (c->Filter120)	av_free((void*)c->Filter120);
   	if (c->Filter121)	av_free((void*)c->Filter121);
   	if (c)	   		av_freep(&c);
}
...
#endif
    switch(c->filter_length)
    {
      case 55:    c->filter_bank= c->Filter55; break;
      case 56:    c->filter_bank= c->Filter56; break;
      case 60:    c->filter_bank= c->Filter60; break;
      case 61:    c->filter_bank= c->Filter61; break;
      case 120:   c->filter_bank= c->Filter120; break;
      case 121:   c->filter_bank= c->Filter121; break;
      default:   printf("\n ERROR: RESAMPLER: Not implemented Filter Bank\n");  break;
    }
/**/
/**/    c->src_incr= out_rate;
...
FELEM *MakeFilter (int out_rate, int in_rate, int filter_size, int phase_shift, int linear, double cutoff)
{
    FELEM *FilterBank;
    double factor= FFMIN(out_rate * cutoff / in_rate, 1.0);
    int phase_count= 1<<phase_shift;
...
#if USE_MATH_H
    filter_length= FFMAX((int)ceil(filter_size/factor), 1);
#else
    filter_length= FFMAX((int)ceil_f(filter_size/factor), 1);
#endif

    FilterBank= av_mallocz(filter_length*(phase_count+1)*sizeof(FELEM));
    av_build_filter(FilterBank, factor, filter_length, phase_count, 1<<FILTER_SHIFT, WINDOW_TYPE);
    memcpy(&FilterBank[filter_length*phase_count+1], FilterBank, (filter_length-1)*sizeof(FELEM));
    FilterBank[filter_length*phase_count]= FilterBank[filter_length - 1];
...
{
    #define TAPS  16
    c->Filter55 = MakeFilter(16000, 44100-1, TAPS, 8, 0, 0.8) ; // 100
    c->Filter56 = MakeFilter(16000, 44100+1, TAPS, 8, 0, 0.8) ;
    c->Filter60 = MakeFilter(16000, 48000-1, TAPS, 8, 0, 0.8) ;
    c->Filter61 = MakeFilter(16000, 48000+1, TAPS, 8, 0, 0.8) ;
    c->Filter120= MakeFilter(16000, 96000-1, TAPS, 8, 0, 0.8) ;
    c->Filter121= MakeFilter(16000, 96000+1, TAPS, 8, 0, 0.8) ;
}

void av_resample_reinit(AVResampleContext *c, float out_rate, float in_rate, int filter_size, int phase_shift, int linear, double cutoff)
{
    double factor= FFMIN(out_rate * cutoff / in_rate, 1.0);
    int phase_count= 1<<phase_shift;
