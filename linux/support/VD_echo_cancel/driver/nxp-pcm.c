...
#define	CFG_SND_PCM_SKIP_1ST_FRAME				true
#define	CFG_SND_PCM_CAPTURE_RESAMPLE_REINIT
//#define	CFG_SND_PCM_CAPTURE_SAMPLE_DETECT
#define	CFG_SND_PCM_CAPTURE_RESAMPLE_PADDING
...
static int pcm_in_sample_rate[3] = {
		CFG_SND_PCM_CAPTURE_INPUT_RATE,
		CFG_SND_PCM_CAPTURE_INPUT_RATE,
		CFG_SND_PCM_CAPTURE_INPUT_RATE
};
...
static DEFINE_MUTEX(snd_pcm_mutex);
static DEFINE_SPINLOCK(snd_pcm_lock);
...
static int nxp_pcm_dma_capture_resample(void *data)
{
	...
	int out_frames;
#endif
	u64 total_frames = 0, expire = 0, total_times = 0, delta_times = 0;
	u_long jitter;
	...
	while (!kthread_should_stop()) {
		...
		if (prtd->rate_changed &&
			...
			printk("RATE CHANE EVENT [%s][%6d.%9ld]\n",
				prtd->dma_param->dma_ch_name, (int)ts.tv_sec, (long)ts.tv_nsec);

			sprintf(message, "SAMPLERATE_CHANGED=%d", prtd->rate_changed);
		...
		#ifdef CFG_SND_PCM_CAPTURE_RESAMPLE_REINIT
		if (prtd->reinit_reamsple) {
			audio_resample_reinit(prtd->resampler, (float)prtd->output_rate, (float)prtd->input_rate);
			prtd->reinit_reamsple = false;
		}
		#endif
...
#ifdef CFG_SND_PCM_CAPTURE_RESAMPLE_PADDING
		total_times = prtd->total_times - delta_times;
		total_frames += out_frames;

		expire = SAMPLE_FRAME_NS_16000 * total_frames;
		jitter = abs(expire - total_times);
//		if (total_frames > 2048 && jitter > SAMPLE_FRAME_NS_16000) {
		if (jitter > SAMPLE_FRAME_NS_16000) {
			printk("!\n");
//			printk("(%4d)[%12llu][%6d:%12ld][%012llu:%012llu] [%d hz] [%6llu][%12llu]\n",
//				current->pid, total_frames, SAMPLE_FRAME_NS_16000, jitter, expire, total_times, (int)prtd->input_rate, prtd->total_counts, prtd->total_times);
			if (expire > total_times) {
				RESAMPLE_INC(out_bytes, frame_bytes, buffer_bytes);
				RESAMPLE_DEC(prtd->dma_avail_size, frame_bytes, buffer_bytes);
			} else {
				RESAMPLE_DEC(out_bytes, frame_bytes, buffer_bytes);
				RESAMPLE_INC(prtd->dma_avail_size, frame_bytes, buffer_bytes);
			}
			/* reset */
			total_frames = 0, delta_times = prtd->total_times;
		}
#endif
		...
		if (prtd->sample_offset >= buffer_bytes) {
			if (prtd->sample_offset > buffer_bytes)
				printk("ERROR: sample over write (%6d over %6d)\n",
					 prtd->sample_offset, buffer_bytes);
			prtd->sample_offset = 0;
		}
...
static void nxp_pcm_dma_previous_clear(struct snd_pcm_substream *substream)
{
	struct nxp_pcm_runtime_data *prtd = substream_to_prtd(substream);
	unsigned offset = prtd->dma_offset;
	int length = snd_pcm_lib_period_bytes(substream);
	void *addr;

	if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE)||
		(prtd->dma_chan->chan_id < DMA_PERIPHERAL_ID_I2S0_TX) ||
	...
	offset -= length;
 	...
 	addr = (void*)(substream->runtime->dma_area + offset);
...
//	printk("(%s)\n", prtd->dma_param->dma_ch_name);
		...
		substream->stream == SNDRV_PCM_STREAM_CAPTURE)
	{
		...
		int  pos, i = 0;
		bool re_resample = false;
		float rate_hz;

		get_time_stamp(tm, ts);

		if (prtd->skip_1st_frame && prtd->first_frame) {
			prtd->first_frame = false;
			goto skip_jitter;
		}
		...
		prtd->total_times += jt;
		totals = prtd->skip_1st_frame ? totals - 1 : totals - 0;
		...
		if (1 == prtd->total_counts)
			printk("1st sample [%s][%6d.%9ld] %d hz\n",
				prtd->dma_param->dma_ch_name, (int)ts->tv_sec, (long)ts->tv_nsec, (int)rate_hz);

		/*
		 * detect samplerate change
		 */
		if (abs(prtd->input_rate - rate_hz) > 50) {
			printk("RATE %d (%6llu:%6llu:%012llu)[%6d]->[%6d] (%s:%d)\n",
				prtd->rate_detects, totals, jt, prtd->total_times,
				(int)prtd->input_rate, (int)rate_hz,
				prtd->dma_param->dma_ch_name, prtd->dma_chan->chan_id);

			/* set new sample rate value */
//			prtd->input_rate = rate_hz;
#if 0
			prtd->trans_period = 0;
			prtd->total_counts = 0;
			prtd->total_times = 0;
#endif
			if (1 > prtd->rate_detects++)
				return;

#ifdef CFG_SND_PCM_CAPTURE_SAMPLE_DETECT
			if (abs(rate_hz - pcm_in_sample_rate[prtd->devno]) > 50) {
				pcm_in_sample_rate[prtd->devno] = prtd->input_rate;
				prtd->rate_changed = true;
				goto skip_jitter;
			}
#else
			return;	// ignore unknown sample
#endif
		}
		 ...
		 *	Filter resmple rate
		 */
		div_s64_rem(totals, FILTER_DEPTH, &pos);
		prtd->jitter_table[pos] = jt;

		if (totals >= FILTER_DEPTH) {
			memcpy(&prtd->jitter_table[1], &prtd->jitter_table[0],
				sizeof(unsigned long) * (FILTER_DEPTH-1));
			prtd->jitter_table[0]= jt;

			for (i=0; FILTER_DEPTH > i; i++)
				sum += prtd->jitter_table[i];

			prtd->current_rate = (int)SAMPLE_RATE_HZ(prtd->period_size*F_DIV,
						div_u64(sum, FILTER_DEPTH));
			prtd->current_rate /= F_DIV;
			re_resample = abs(prtd->current_rate - prtd->input_rate) ? true : false;

			if (re_resample) {
				prtd->reinit_reamsple = true;
				#if 0
				printk("(%6llu)[%09llu][%2d][%6d.%d->%6d.%d](%s)\n",
					totals, jt, pos, (int)prtd->input_rate, (int)(prtd->input_rate*F_DIV)%F_DIV,
					(int)prtd->current_rate, (int)(prtd->current_rate*F_DIV)%F_DIV,
					prtd->reinit_reamsple?"O":"X");
				#endif
				prtd->input_rate = prtd->current_rate;
			}
		}
	#endif
	}
...
skip_jitter:
	...
	nxp_pcm_dma_previous_clear(substream);
	snd_pcm_period_elapsed(substream);
}
...
static int nxp_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (prtd->dma_chan->chan_id == DMA_PERIPHERAL_ID_I2S0_RX ||
		...
		pr_debug("***[%s:%d %d %s resampler ON]***\n",
			__func__, __LINE__, prtd->dma_chan->chan_id, prtd->dma_param->dma_ch_name);
	}
					...
					struct snd_pcm_hw_params *params)
{
	...
	prtd->skip_1st_frame = CFG_SND_PCM_SKIP_1ST_FRAME;
	prtd->first_frame = true;
	prtd->rate_changed = false;
	prtd->rate_detects = 0;
	prtd->ts = ts;
	prtd->devno =
	/* resample rate */
	prtd->reinit_reamsple = false;
	prtd->input_rate = CFG_SND_PCM_CAPTURE_INPUT_RATE;
//	prtd->input_rate = pcm_in_sample_rate[prtd->devno];
	prtd->output_rate = CFG_SND_PCM_CAPTURE_RESAMPLE_HZ;

	pcm_rate_chaned_event_up = false;

	mutex_lock(&snd_pcm_mutex);
	if (prtd->run_resampler) {
		...
		prtd->task = p;
		printk("%s %s create resampler task (ch:%d %d->%d) (0x%p)...\n",
			__func__, STREAM_STR(substream->stream),
			prtd->channels, (int)prtd->input_rate, (int)prtd->output_rate,
			prtd->resampler);
	}
	mutex_unlock(&snd_pcm_mutex);
...
static int nxp_pcm_hw_free(struct snd_pcm_substream *substream)
{
	...
	pr_debug("%s: %d\n", STREAM_STR(substream->stream), prtd->task_running);
