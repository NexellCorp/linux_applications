...
#endif
	printf("raw sample open : %u ch, %d hz, %u bits\n", channels, rate, bits);
				...
				int channels, int bits, unsigned long bytes)
{
	...
	printf("raw sample close: %u ch, %d hz, %u bits, %lu bytes\n",
...
#define	CMD_CAPTURE				(1<<2)
#define	CMD_AEC_PROCESS			(1<<3)
#define	CMD_REINIT				(1<<4)
#define	CMD_CAPTURE_PAUSE		(1<<5)
...
static void *audio_pdm_capture(void *data)
{
	...
	int per_2ch_bytes = period_bytes;
	bool err;
	...
	char path[256];
	struct wav_header header, header2;

	unsigned char *PcmBuf, *Buf0, *Buf1;

	if (stream->command & CMD_CAPTURE) {
		sprintf(path, "%s/pdm0.raw", stream->file_path);
		stream->fp = open_wave(path, 2, sample_rate, sample_bits, &header);
		if (NULL == stream->fp) {
			printf("E: %s open failed for wave...\n", path);
			goto th_exit;
		}
		if (4 == channels) {
			sprintf(path, "%s/pdm1.raw", stream->file_path);
			stream->fp2 = open_wave(path, 2, sample_rate, sample_bits, &header2);
			if (NULL == stream->fp2) {
				printf("E: %s open failed for wave...\n", path);
				goto th_exit;
			}
...
pdm_reinit:
	...
	per_2ch_bytes = period_bytes = pRec->GetPeriodBytes();
	if (pQbuf0 && pQbuf1)
		per_2ch_bytes /= 2;
	...
	while (!(CMD_EXIT & stream->command) &&
		...
		/* deliver */
		Buf0 = pQbuf0->PushBuffer(per_2ch_bytes, wait);
		Buf1 = pQbuf1->PushBuffer(per_2ch_bytes, wait);

		if (Buf0 && Buf1) {
			unsigned int *s  = (unsigned int *)PcmBuf;
			unsigned int *d0 = (unsigned int *)Buf0;
			unsigned int *d1 = (unsigned int *)Buf1;
		#if 1
		...
		#endif
#ifdef SUPPORT_PDM_AGC
			agc_Agc(&agc_st[0], (short *)(Buf0),    2, -30);
			agc_Agc(&agc_st[1], (short *)(Buf0+2),  2, -30);
			agc_Agc(&agc_st[2], (short *)(Buf1),    2, -30);
			agc_Agc(&agc_st[3], (short *)(Buf1+2),  2, -30);
#endif
			pQbuf0->Push(per_2ch_bytes);
			pQbuf1->Push(per_2ch_bytes);
		}
		...
		if (stream->fp &&
			!(CMD_CAPTURE_PAUSE & stream->command)) {
			write_wave(stream->fp , Buf0, per_2ch_bytes);
			if (stream->fp2)
				write_wave(stream->fp2, Buf1, per_2ch_bytes);
			read_bytes += per_2ch_bytes;
		}
	}
...
th_exit:
	if (stream->fp)
		close_wave(stream->fp, &header, 2, sample_bits, read_bytes);

	if (stream->fp2)
		close_wave(stream->fp2, &header2, 2, sample_bits, read_bytes);
...
static void *audio_i2s_capture(void *data)
{
	...
	if (stream->command & CMD_CAPTURE) {
		sprintf(path, "%s/i2s%d.%d.wav", stream->file_path, card, device);
...
i2s_reinit:
	...
	while (!(CMD_EXIT & stream->command) &&
		...
		if (stream->fp &&
			!(CMD_CAPTURE_PAUSE & stream->command)) {
			write_wave(stream->fp, Buffer, period_bytes);
			read_bytes += period_bytes;
		}
	}
...
th_exit:
	if (stream->fp)
		close_wave(stream->fp, &header, channels, sample_bits, read_bytes);
...
static void *audio_playback(void *data)
{
	...
	int period_bytes = stream->period_bytes;
	bool err;
	int qbuffers = 0, i = 0, ret = 0;
	int wait = STREAM_WAIT_TIME;
	...
	short *In_Buf1, *In_Buf2;	// PDM
	short *In_Ref1, *In_Ref2;	// I2S
	short out_PCM_aec1[512], out_PCM_aec2[512];
	short out_PCM[512];

	if (stream->command & CMD_CAPTURE) {
		sprintf(path, "%s/out.wav", stream->file_path);
		stream->fp = open_wave(path, channels, sample_rate, sample_bits, &header);
		if (NULL == stream->fp) {
			printf("E: %s open failed for wave...\n", path);
			goto th_exit;
...
//	set_new_scheduler(getpid(), SCHED_FIFO, 99);
	...
	while (!(CMD_EXIT & stream->command)) {
		...
		/* get buffers */
		for (i = 0; qbuffers > i; i++)
			Buffer[i] = pQbuf[i]->PopBuffer(PCM_QBUF_FRAMEBYTES, wait);
		...
		if (CMD_EC_PROCESS & stream->command) {
			...
			END_TIMESTAMP_US(ts, td);
	//		printf("AEC [%3llu.%03llu] %d\n", td/1000, td%1000, pcm_channel);

			if (0 == pcm_channel) {
				if (pPlay)
					ret = pPlay->PlayBack(out_PCM, PCM_QBUF_FRAMEBYTES);
			} else {
				if (Buffer[pcm_channel-1])
					if (pPlay)
						ret = pPlay->PlayBack(Buffer[pcm_channel-1], PCM_QBUF_FRAMEBYTES);
			}
		} else {
			if (Buffer[pcm_channel-1])
				if (pPlay)
		...
		if (stream->fp &&
			!(CMD_CAPTURE_PAUSE & stream->command)) {
			write_wave(stream->fp, out_PCM, PCM_QBUF_FRAMEBYTES);
			play_bytes += PCM_QBUF_FRAMEBYTES;
		}

		/* release buffers */
		for (i = 0; qbuffers > i; i++)
			pQbuf[i]->Pop(PCM_QBUF_FRAMEBYTES);
...
th_exit:
	if (stream->fp)
		close_wave(stream->fp, &header, channels, sample_bits, play_bytes);
...
int main(int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, "hnso:c:"))) {
		switch(opt) {
...
//	audio_init_stream(&streams[3], 3, 0, "default:CARD=SNDNULL1"   	, STREAM_CAPTURE_I2S, 2, 16000, 16, PCM_PERIOD_BYTES  , 64, &qcounts);	// I2S1
	...
	for (i = 0; NR_THREADS > i; i++) {
		...
		stream->command |= capture ? CMD_CAPTURE : 0;
	...
	//	fflush(stdout);
		...
		if (0 == strncmp("p", cmd, strlen(cmd)-1)) {
			stream_command(CMD_CAPTURE_PAUSE, true, thread, streams);
			continue;
		}

		if (0 == strncmp("r", cmd, strlen(cmd)-1)) {
			stream_command(CMD_CAPTURE_PAUSE, false, thread, streams);
			continue;
		}
