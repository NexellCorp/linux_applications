...
static int nxp_pdm_check_param(struct nxp_pdm_snd_param *par)
{
	...
	printk("PDM sample_rate : %dKhz\n", par->sample_rate);
	switch (par->sample_rate) {
		case 16000:
			// if 16 kHZ
			#if 0 // PCLK 204.8
			NumOfClock = 50;     // 200 Mhz / 49 / 2 = 2.04 Mhz
			OverSampleRate = 128; // 2.04 Mhz / 128 = 15.9 kHz
			#else // PCLK 200
			NumOfClock = 50;     // 200 Mhz / 49 / 2 = 2.04 Mhz
			OverSampleRate = 125; // 2.04 Mhz / 128 = 15.9 kHz
			#endif
...
#if 0
	writel(0x007c0004, (base+0x00));
	writel(0x00003119, (base+0x14));

	writel(0x00D6006B, (base+0x04));
	writel(0xFF2AFF95, (base+0x08));
	writel(0x3F02E0FD, (base+0x0c));
#else
	pdm->CTRL = 0x000f0004;
	writel(0x000f0004, (base+0x00));
	writel(0x00003119, (base+0x14));

//	writel(0x007D003E, (base+0x04));
//	writel(0xFF83FFC2, (base+0x08));
//	writel(0x3E94E164, (base+0x0c));
	writel(0x007D003E, (base+0x04));
	writel(0xFF83FFC2, (base+0x08));
	writel(0x3E94E164, (base+0x0c));
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	struct nxp_pcm_dma_param *dma_param =
		(stream == SNDRV_PCM_STREAM_PLAYBACK ? NULL : &par->dma);
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		if (SNDRV_PCM_STREAM_CAPTURE == stream)
			nxp_snd_dev_trigger(substream, cmd, type, dma_param, base, 0);
		else
		#endif
				 ...
				 struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	pr_debug("%s: %s format=0x%x, channel=%d\n",
		__func__, STREAM_STR(substream->stream), format, channels);
